...
class GameResponseAssemblerServiceTest {
    ...
    @Mock private WsConnectionRegistry connectionRegistry;
    ...
    @BeforeEach
    void setUp() {
        assembler = new GameResponseAssemblerService(userRepository, connectionRegistry);
    ...
    private GameOver buildGameOver() {
        ...
        when(winnerPlayer.getActiveEffects()).thenReturn(com.letraaletra.api.features.player.domain.effect.PlayerActiveEffects.create());
        when(loserPlayer.getActiveEffects()).thenReturn(com.letraaletra.api.features.player.domain.effect.PlayerActiveEffects.create());
    ...
    void shouldAssembleRankingResponseFromProvidedDeltasWithoutBusinessSideEffects() {
        ...
        verify(connectionRegistry).findByUserId(winnerUser.getUserId());
        verify(connectionRegistry).findByUserId(loserUser.getUserId());
        verifyNoMoreInteractions(userRepository, connectionRegistry, game);
    }
    ...
    void shouldAssembleDefaultResponseForCustomMatch() {
        ...
        verify(connectionRegistry).findByUserId(winnerUser.getUserId());
        verify(connectionRegistry).findByUserId(loserUser.getUserId());
        verifyNoMoreInteractions(connectionRegistry, game);
    }
}
