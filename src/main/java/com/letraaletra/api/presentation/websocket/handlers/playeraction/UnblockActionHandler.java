...
@Component
public class UnblockActionHandler implements InGameActionHandler<UnblockActionDTO> {
    @Autowired
    ...
    @Override
    public void handle(UnblockActionDTO request, WebSocketSession session, String tokenGameId) {
        String userId = (String) session.getAttributes().get("userId");

        Position position = new Position(request.position().x(), request.position().y());
        GameAction action = new UnblockCellAction(position, request.actionId());
