...
public class FindActiveGameByRoomNameMapper {
    private static final Set<String> ALLOWED_SORTS = Set.of("roomName", "roomCode", "status");

    public static FindActiveGameByRoomNameInput toInput(AuthenticatedUser principal, String roomName, Pageable pageable) {
        Pageable pages = Pageables.sanitize(pageable, ALLOWED_SORTS, Sort.unsorted());

        return new FindActiveGameByRoomNameInput(
                principal,
                roomName,
                pages.getPageNumber(),
                pages.getPageSize(),
    ...
    public static PageResponse<GameResponse> toResponse(FindActiveGameByRoomNameOutput output) {
